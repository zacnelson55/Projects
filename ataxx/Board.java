...
class Board {
    ...
    /** Number of defualt open spots. */
    static final int DEFAULT_OPEN = 47;

    /** Number of board spots. */
    static final int BOARD_SPOTS = 121;

    /** A new, cleared board in the initial configuration. */
    Board() {
        _board = new PieceColor[EXTENDED_SIDE * EXTENDED_SIDE];
        _totalOpen = DEFAULT_OPEN;
    ...
    void clear() {
        _whoseMove = RED;
        for (int i = 0; i < BOARD_SPOTS; i++) {
            unrecordedSet(i, BLOCKED);
        }
        for (char col = 'a';  col < (char) ('g' + 1); col++) {
            for (char row = '1'; row < (char) ('7' + 1); row++) {
    ...
    void makeMove(Move move) {
        ...
        for (int c = -1; c < 2; c++) {
            for (int r = -1; r < 2; r++) {
                int side = neighbor(index(move.col1(), move.row1()), c, r);
                if (get(side) == opponent) {
                    set(side, whoseMove());
                }
            }
        }
    ...
    /** Undo the last move. */
    void undo() {
        while (_undoSquares.peek() != null && !_undoPieces.empty()) {
        ...
        int lastMoveInd = _allMoves.size() - 1;
        if (_allMoves.get(lastMoveInd).isJump()) {
            _numJumps--;
        }
    ...
    void setBlock(char c, char r) {
        ...
        if (get((char) ('d' - colDist), r) == EMPTY) {
            unrecordedSet((char) ('d' - colDist), r, BLOCKED);
            _totalOpen--;
        }
        if (get(c, (char) ('4' - rowDist)) == EMPTY) {
            unrecordedSet(c, (char) ('4' - rowDist), BLOCKED);
            _totalOpen--;
        }
        if (get((char) ('d' - colDist), (char) ('4' - rowDist)) == EMPTY) {
            unrecordedSet(
                    (char) ('d' - colDist), (char) ('4' - rowDist), BLOCKED);
            _totalOpen--;
        }
